...
public class Inventory
{
    ...
    public bool Add(Item item, int amount, bool send) {
        ...
        if (item.Property == ItemProperty.Unique && Any(item)) {
            if (send)
                _player.Client.Tcp.Send(new MessageError(MessageError.MessageErrorType.uniqueItem));
            return false;
        }
        ...
        // inventory full?
            ...
            if (send)
                _player.Client.Tcp.Send(new MessageError(MessageError.MessageErrorType.inventoryFull));
            return false;
        }
        // doesn't have any?
        else {
